...
## Known risks (agreed)
- OAuth logs currently include headers, request params, and tokens (risk of credential leakage in logs).
- OAuth authorization logs include redirect URLs, state, and code fragments (sensitive when combined).
- Legacy bearer auth is static and never expires (risk of long-lived credential exposure).
- Dynamic client registration is open to any caller (risk of unwanted clients / DB growth).
